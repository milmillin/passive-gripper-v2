...
void DebugSubdivision(const PassiveGripper& psg) {
  ...
  for (size_t i = 10; i <= 38; i++) {
    ...
    double cost = ComputeCost(
        psg.GetParams(), settings, psg.GetMDR(), dCost_dParam, nullptr);
...
void DebugCost(const PassiveGripper& psg, Debugger& debugger) {
  ...
  double cost = ComputeCost1(
      psg.GetParams(), psg.GetSettings(), psg.GetMDR(), grad_, &debugger);
...
void DebugGradient(const PassiveGripper& psg) {
  ...
  GripperParams grad_;
  double org_cost = ComputeCost1(
      psg.GetParams(), psg.GetSettings(), psg.GetMDR(), grad_, nullptr);
  MyFlattenGrad(grad_, grad.data());
  ...
  for (int i = 0; i < n; i++) {
    ...
    double cur_cost = ComputeCost1(
        cur_, psg.GetSettings(), psg.GetMDR(), unused_grad_, nullptr);
