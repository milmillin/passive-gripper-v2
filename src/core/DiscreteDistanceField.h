...
class DiscreteDistanceField {
 public:
  ...
  double& getVoxel(int x, int y, int z) {
    return distance[x * size(1) * size(2) + y * size(2) + z];
  }

  const double& getVoxel(int x, int y, int z) const {
    return distance[x * size(1) * size(2) + y * size(2) + z];
  }

  double& getVoxel(Eigen::Vector3i coord) {
    return getVoxel(coord(0), coord(1), coord(2));
  }

  const double& getVoxel(Eigen::Vector3i coord) const {
    return getVoxel(coord(0), coord(1), coord(2));
  }

  double& getVoxel(Eigen::Vector3d coord) {
  ...
  const double& getVoxel(Eigen::Vector3d coord) const {
    coord = (coord - lower_bound) / resolution;
    Eigen::Vector3i coordi = coord.cast<int>();
    for (int i = 0; i < 10; i++)
      for (int dx = -i; dx <= i; dx++)
